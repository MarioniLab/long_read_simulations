...
def simulate(args, output=sys.stderr):
    ...
    for i in range(round(target_size / ref_size)):
        for ref_name in ref_seqs.keys():
            ref_seq = {ref_name: ref_seqs[ref_name]}
            rev_comp_ref_seq = {ref_name: rev_comp_ref_seqs[ref_name]}
            ref_contig = [ref_name]
            fragment, info = build_fragment(frag_lengths, ref_seq, rev_comp_ref_seq, ref_contig,
            ...
            info = ' '.join(info)
            print(f'@{read_info},{read_name} {info}')
            print(seq)
            print('+')
            print(quals)
